...
export type Registries = Record<Publisher, RegistryDetails[]>;
...
export async function getPartnerRegistries(
	env: Env,
	results: Registries,
	publisher: number,
) {
	const match = await env.MAYOI_KV.get(`last_fetched_registry_${publisher}`);
	let latestRegistry;
	...
	page_loop: for (let i = 1; i <= MAX_PAGE; i++) {
		...
		if (response.ok) {
			try {
				...
				for (const row of $("#list_data_return table tbody tr")) {
					...
					if (partner.includes("IPM")) {
						...
						partner.includes("X.Y.Z") ||
						partner.includes("Amak") ||
						partner.includes("AMAK")
					) {
...
export async function getPublisherRegistries(
	env: Env,
	results: Registries,
	publisher: number,
) {
